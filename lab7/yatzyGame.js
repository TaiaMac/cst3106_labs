...
roller.addEventListener("click",()=>{
  ...
  // fetch dice values from server
  fetch("http://localhost:3000/roll-dices")
    .then(res => res.json())
    .then(data => {
      const serverDice = data.dice;
      // apply only to unheld dice
      engine.dice = engine.dice.map((val, i) =>
        engine.held[i] ? val : serverDice[i]
      );
      engine.rollsLeft--;
      engine.autoScore();
    ...
    .catch(err => {
      console.error(err);
      message.textContent = "Server error rolling dice. Try again.";
    });
});
